                ...
                if (scriptTag) {
                    ...
                    if (remixContextMatch) {
                        ...
                        if (remixContext.state && remixContext.state.loaderData && remixContext.state.loaderData['routes/$url_token'].jobPosts) {
                            const jobPosts = remixContext.state.loaderData['routes/$url_token'].jobPosts.data;
                            
                            const extractedJobs = jobPosts.map(job => ({
                                job_id: job.id,
                                title: job.title,
...
export const filterGreenHouseJobs = async (embed) => {
    ...
    const filterJobData = async (data) => {
        ...
        if (location_matched) {
            ...
            if (title_matched) {
                // let posting_date = await getJobPostingDates(data["job_link"],data["company_name"]);
                let posting_date = await getJobPublishedAt(data.job_link)
